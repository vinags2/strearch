...
export async function saveAccessToken(saved_access_token: string, saved_refresh_token: string) {
    ...
    return await async_axios({ url: route_to_save_token, flag: 2, method: 'post' });
}

export async function getStats() {
    return await async_axios({ url: route('stats'), flag: 8 });
}

export async function getChartData() {
    return await async_axios({ url: route('chartData.get'), flag: 22 });
}

export async function getAthlete() {
    return await async_axios({ url: route('athlete.get'), flag: 11 });
}

export async function save_athlete(athlete: any) {
    return await async_axios({ url: route('athlete.save', athlete.id), flag: 6, method: 'post', post_data: athlete });
}

export async function save_activity(activity: any) {
    return await async_axios({ url: route('activity.save', activity.id), flag: 6, method: 'post', post_data: activity });
}
